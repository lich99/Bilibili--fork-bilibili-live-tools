...
async def handle_1_TV_raffle(type, num, real_roomid, raffleid):
    ...
    if json_response2['code'] == 0:
        ...
        Printer().printer(f'ID {raffleid:<9} {award}', 'Lottery', 'cyan')
        with open(r'./stat/daliy_log.txt','a') as f:
            f.write('[{}] {}'.format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), award + '\n'))
        with open(r'./stat/daliy_total.txt','r') as f:
            i = -1
            while award[i] in string.digits:
                i -= 1
            update = int(award[i+1:])
            amount = int(f.readline())
        with open(r'./stat/daliy_total.txt','w') as f:
            f.write(str(update + amount))
    else:
...
class bilibiliClient():
    ...
    async def connectServer(self):
        try:
            ...
            await asyncio.sleep(5 + random.uniform(0.1, 10))
            return
        self._reader = reader
        self._writer = writer
    ...
    async def HeartbeatLoop(self):
        while not self.connected:
            await asyncio.sleep(0.5 + random.uniform(0.1, 2))
    ...
    async def ReadSocketData(self, len_wanted):
        ...
        while len_remain != 0:
            try:
                ...
                await asyncio.sleep(1 + random.uniform(0.1, 1))
                return None
